...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (b.action === 'dismiss') { await db.update(b.id, { status: 'dismissed' }); return json({ ok: true }, 200); }
    if (b.action === 'reopen') { await db.update(b.id, { status: 'open' }); return json({ ok: true }, 200); }
