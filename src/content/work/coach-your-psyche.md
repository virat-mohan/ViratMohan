...
### Built for ongoing use, not a single session

- Four personality-styled AI therapists, so the person choosing can find a style that actually fits them.
- Full session history — conversations pick up with context, not from zero each time.
- Pay-per-message billing via Stripe, instead of a flat fee that punishes light use or caps heavy use.
