...
CRITICAL RULE — simulate one concrete scenario, engineer-style: do not build an abstract diagram of the mechanism — pick ONE specific, plausible instance of the problem (a named order, invoice, guest enquiry, applicant, whatever the domain calls for — invent a realistic ID/name consistent with Step 3's assumptions) and play out exactly what happens to it, step by step, matching Step 4's how_it_works_steps in order:
1. DATA IN: show it being pulled from the actual named tool in that step's trigger_or_data_source (HubSpot, Shopify, Zendesk, a spreadsheet — whatever the client actually listed, or a sensible default if none given) — a visible record/row/card with realistic fields, not a placeholder box.
2. PROCESSING: show the mechanism actually reasoning over that data using the selected framework's own logic/vocabulary from Step 2 — the specific check, score, or rule being applied, not a generic spinner.
3. OUTCOME: the run must end in ONE of two concrete, visibly different outcomes depending on what the scenario calls for — (a) a direct output (the record updated, a message sent, a value corrected) shown as a finished artifact the visitor can read, or (b) an escalation to a named human role on the client's team (e.g. "Escalated to AR Lead — needs manual override") rendered as an actual message — an email/Slack-style card with a subject line and a clear, one-line next action ("Action needed: approve the ₹4,200 variance"). Pick whichever outcome genuinely fits the scenario you invented; do not force every run down the same path if the mechanism would realistically branch.
Build a single "▶ Run the mechanism" play control that AUTOPLAYS this whole sequence on its own timer (roughly 900ms–1.8s per step), no further clicks needed, with a "Run again" replay control after it finishes. Do NOT make the visitor manually click "next" through each step — that reads as a slideshow, not a working system. A manual click-through/inspect control is acceptable only as a secondary affordance for pausing on one step, never as the primary way to experience it.

CRITICAL RULE — agents get their own pane: while the scenario above plays in the main pane, run a separate, persistent "Agent activity" pane alongside it (a sidebar or split panel, not inline text buried in one card) that logs which ACTUAL agent from Step 4's agent_sequence is firing at each step, in real time — e.g. "Data Extraction Agent — reading invoice fields…" then "Reconciliation/Matching Agent — checking against PO #4471…", ending in a clear terminal line ("Escalated to Reconciliation Lead" or "Resolved — no human needed"). Use the agent's exact curated name every time. Also list every DISTINCT agent from that sequence up front in this same pane, visible before Run is ever pressed, so a visitor who never presses Run still sees which agents do the work.

CRITICAL RULE — then show it at scale: after the single-scenario run, the artefact must show what happens when this same mechanism runs on every case like it, all month — a distinct "at scale" view (its own tab, or a clearly separated section reached after the single-scenario run) with the before/after numbers and P&L line from Step 3. This is the payoff: one scenario proves the mechanism actually works, the scaled view proves why it matters.
...
2. "How it connects" — the specific tools/systems from each step's trigger_or_data_source, shown as a simple flow (tool → agent → tool/output), plus the up-front "agents involved" list from the rule above.
3. "See it run" — the concrete single-scenario simulation and Agent activity pane, per the two rules above.
4. "Business impact" — the "at scale" view: the before/after numbers and the specific P&L line from Step 3, reachable directly without replaying the scenario first.
...
- has_run_control: does it have a single primary "Run"/play-style control that autoplays a concrete, named single-case scenario end to end — data in, processing, and a clear output-or-escalation outcome (per the scenario-simulation rule) — rather than requiring manual click-through, and rather than an abstract diagram with nothing concrete running through it? "block" if it's still a manual-click-only slideshow, or if the "scenario" is just generic labels with no specific case (name/ID/record) actually shown.
- before_after_integrity: does the "at scale" business-impact view show BEFORE and AFTER numbers that actually differ and match Step 3's calculation exactly (not a second, independently-invented set of numbers)? "block" if before equals after, or if a live counter's after-value never actually updates when the run sequence completes.
