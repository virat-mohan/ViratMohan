...
import { calculateAmcPricing, estimateDirectTechCost, AMC_MONTHLY_MINIMUM_INR, type AmcRateBenchmark, type AmcResourceEstimate } from '../../src/lib/amc';

function profile(overrides: { domain?: string; mechanism_type?: any; integration_count?: number } = {}) {
  return { domain: 'general', mechanism_type: 'automation_pipeline' as const, integration_count: 1, ...overrides };
}
...
describe('calculateAmcPricing', () => {
  it('never returns below the ₹50,000/month minimum floor, even for a tiny estimate', () => {
    ...
    const result = calculateAmcPricing(profile(), tinyEstimate, benchmarks);
  ...
  it('prices above the floor for a realistic multi-category estimate, using benchmarked rates', () => {
    const result = calculateAmcPricing(profile(), fullEstimate, benchmarks);
  ...
  it('flags a resource category as unverified when its benchmark rate is not yet verified', () => {
    ...
    const result = calculateAmcPricing(profile(), fullEstimate, partiallyUnverified);
  ...
  it('does not fabricate a rate when no benchmark exists for a category — falls back to a cost-based floor and flags it', () => {
    ...
    const result = calculateAmcPricing(profile(), fullEstimate, missingSme);
    expect(result.unverified_rate_categories).toContain('sme');
  ...
  it('falls back to a domain-specific rate over "general" when both exist', () => {
    ...
    const generalResult = calculateAmcPricing(profile(), fullEstimate, withEcommerce);
    const ecommerceResult = calculateAmcPricing(profile({ domain: 'e-commerce' }), fullEstimate, withEcommerce);
    expect(ecommerceResult.market_equivalent_value_low_inr).toBeGreaterThan(generalResult.market_equivalent_value_low_inr);
  });

  it('folds a benchmarked direct-technology cost into the recommendation, separate from labor', () => {
    const result = calculateAmcPricing(profile(), fullEstimate, benchmarks);
    expect(result.direct_tech_cost_low_inr).toBeGreaterThan(0);
    expect(result.direct_tech_cost_high_inr).toBeGreaterThan(result.direct_tech_cost_low_inr);
    expect(result.market_equivalent_value_low_inr).toBeGreaterThanOrEqual(result.direct_tech_cost_low_inr);
  });
});

describe('estimateDirectTechCost', () => {
  it('applies roughly a ±10% variance band around the benchmarked baseline', () => {
    const result = estimateDirectTechCost({ mechanism_type: 'automation_pipeline', integration_count: 2 });
    const mid = (result.low_inr + result.high_inr) / 2;
    expect(result.low_inr).toBeCloseTo(mid * 0.9, -1);
    expect(result.high_inr).toBeCloseTo(mid * 1.1, -1);
  });

  it('costs more for an agent-workflow mechanism than a simple analytics dashboard', () => {
    const agent = estimateDirectTechCost({ mechanism_type: 'agent_workflow', integration_count: 1 });
    const dashboard = estimateDirectTechCost({ mechanism_type: 'analytics_dashboard', integration_count: 1 });
    expect(agent.low_inr).toBeGreaterThan(dashboard.low_inr);
  });

  it('increases with more integrations', () => {
    const fewer = estimateDirectTechCost({ mechanism_type: 'data_integration', integration_count: 1 });
    const more = estimateDirectTechCost({ mechanism_type: 'data_integration', integration_count: 5 });
    expect(more.low_inr).toBeGreaterThan(fewer.low_inr);
  });

  it('never claims to be metered — basis text says benchmarked', () => {
    const result = estimateDirectTechCost({ mechanism_type: 'agent_workflow', integration_count: 0 });
    expect(result.basis.toLowerCase()).toContain('benchmarked');
  });
});
